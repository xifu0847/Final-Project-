...
def parse_input(input_file):
	...
	with open(input_file) as f:
		for line in f:
			...
			gene_obj.total_diff = gene_obj.fast_of + gene_obj.fast_off + gene_obj.fast_dpf + (
				gene_obj.of_off + gene_obj.of_dpf + gene_obj.off_dpf)
			gene.append(gene_obj)

	return header, gene # header = ['gene_id', 'gene_name', 'OF'] gene = [Gene1, Gene2, ...]

# Gene.name
def NCBI_url(gene_name):
	URL_name = 'https://eutils.ncbi.nlm.nih.gov/entrez/eutils/esearch.fcgi?db=gene&term='
	URL_info = 'https://eutils.ncbi.nlm.nih.gov/entrez/eutils/esummary.fcgi?db=gene&id='
...
def write_data(neg_list, pos_list, name1, name2, env):
	...
	sheet = annotation_file.add_sheet(name1 + " v.s. " + name2 + " POS", cell_overwrite_ok=True)
	sheet.write(0, 0, "UID")
	...
	sheet = annotation_file.add_sheet(name1 + " v.s. " + name2 + " NEG", cell_overwrite_ok=True)
	sheet.write(0, 0, "UID")
	sheet.write(0, 1, "Name")
	sheet.write(0, 2, "Description")
	sheet.write(0, 3, "Summary")
